...
public class JCDIComponentProviderFactory implements
		IoCComponentProviderFactory, IoCComponentProcessorFactoryInitializer {

	private static final Logger LOGGER = Logger
			.getLogger(JCDIComponentProviderFactory.class.getName());
	...
	public void init(final IoCComponentProcessorFactory cpf) {
		...
		if (e == null) {
			throw new ContainerException("Reference to "
					+ JCDIComponentExtension.class + " is null");
		}
		...
		for (final ProcessInjectionTarget pit : e.getProcessInjectionTargets()) {
			...
			final ComponentScope cs = c.isAnnotationPresent(Provider.class) ? ComponentScope.Singleton
					: s == Dependent.class ? ComponentScope.Undefined
							: getComponentScope(b);

			LOGGER.info("Adapting InjectionTarget for " + c.getName()
					+ " in the scope " + b.getScope());
		...
		if (s == Dependent.class) {
			...
			return new ComponentProviderDestroyable() {
				...
				public Object getInstance() {
					final CreationalContext<?> bcc = bm
	...
	private interface ComponentProviderDestroyable extends
